...
export default function ReNameModalDefault({
  fileName,
  fileToken,
  fileId,
}: {
  ...
  const reName = async () => {
    let newName = nameInputRef.current?.value?.trim();
    if (!newName) {
    ...
    try {
      const response = await fetch(
        `${API_URL}/storage/files/${fileId}/update/`,
        {
          method: "PUT",
          headers: {
      ...
      alert(`Файл успешно переименован в "${newName}"`);
      clearModalContent();
    } catch (error) {
